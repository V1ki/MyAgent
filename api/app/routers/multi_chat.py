...
def get_model_provider_info(db: Session, implementation_id: UUID) -> Dict[str, str]:
    ...
    try:
        implementation = ModelImplementationService.get_implementation(
        ...
        if implementation:
            ...
            return {
                "model_name": (
                    f"{implementation.provider_model_id}"
                    if implementation
                    else implementation.provider_model_id
                ),
    ...
    return {"model_name": "Unknown Model", "provider_name": "Unknown Provider"}
...
async def chat_with_multiple_models(
    ...
    if not model_implementation_ids:
        raise HTTPException(
            status_code=400,
            detail="At least one model implementation must be specified",
        )
    if not message:
    ...
    new_turn = ConversationTurnCreate(
        conversation_id=conversation_id, user_input=message, model_parameters=parameters
    )
    ...
    # build messages
    messages,prompt_token_count = ConversationService.build_message(conversation)
    ...
    saved_responses = ModelOrchestrator.save_model_responses(
        db=db, turn=db_turn, responses=responses, prev_prompt_token_count=prompt_token_count
    )
    ...
    result = {
        ...
        "prompt_token_count": db_turn.prompt_token_count,
        "responses": [
            {
                "id": str(resp.id),
                "content": resp.content,
                "response_metadata": resp.response_metadata,
                "model_implementation_id": str(resp.model_implementation_id),
                ...
                "error": next(
                    (
                        r["error"]
                        for r in responses
                        if r["implementation_id"] == str(resp.model_implementation_id)
                    ),
                    None,
                ),
            }
...
async def stream_chat_with_multiple_models(
    ...
    if not conversation:
        raise HTTPException(
            status_code=404, detail=f"Conversation with ID {conv_id} not found"
        )

    # Create a new conversation turn
    new_turn = ConversationTurnCreate(
        conversation_id=conv_id, user_input=message, model_parameters=params
    )
    db_turn = ConversationTurn(
        conversation_id=new_turn.conversation_id,
        user_input=new_turn.user_input,
    ...
    async def stream_generator():
        ...
        responses = await ModelOrchestrator.orchestrate_model_calls(
            db=db, implementation_ids=model_ids, prompt=message, parameters=params
        )
        ...
        ModelOrchestrator.save_model_responses(db=db, turn=db_turn, responses=responses)
...
async def select_response_as_context(
    turn_id: str, response_id: str, db: Session = Depends(get_db)
):
    """
    ...
    if not turn:
        raise HTTPException(
            status_code=404, detail=f"Conversation turn with ID {turn_id} not found"
        )
    ...
    selected_response = (
        db.query(ModelResponse).filter(ModelResponse.id == response_uuid).first()
    )
